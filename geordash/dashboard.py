...
def ows(stype, url):
    ...
    return render_template('ows.html', s=service, layers=layers, type=stype, url=url.replace('/', '~'), consumers=used_by, previous_jobs=all_jobs_for_ows)
...
def owslayer(stype, url, lname):
    ...
    return render_template('owslayer.html', s=service, type=stype, url=url.replace('/','~'), lname=lname, consumers=used_by, previewqparams=params, localmduuids=localmduuids, previous_jobs=all_jobs_for_owslayer)
...
def map(mapid):
    ...
    return render_template('map.html', mapid=mapid, details=get_res_details(request, m), resources=resc, previous_jobs=all_jobs_for_mapid)
...
def ctx(ctxid):
    ...
    return render_template('ctx.html', ctxid=ctxid, details=get_res_details(request, c), resources=resc, previous_jobs=all_jobs_for_ctxid)
