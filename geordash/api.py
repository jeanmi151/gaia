...
def maps():
    maps = mapstore_get(
        request,
        "rest/geostore/extjs/search/category/MAP/***/thumbnail,details,featured?includeAttributes=true",
    )
    if maps.status_code != 200:
        app.logger.error(
            f"failed getting MAPs from geostore, got {maps.status_code}: {maps.text}"
        )
...
def contexts():
    maps = mapstore_get(
        request,
        "rest/geostore/extjs/search/category/CONTEXT/***/thumbnail,details,featured?includeAttributes=true",
    )
    if maps.status_code != 200:
        app.logger.error(
            f"failed getting CONTEXTs from geostore, got {maps.status_code}: {maps.text}"
        )
        return str(maps.status_code)
    return maps.json()
...
def gninternalid(request, uuid):
    ...
    if "sec-roles" in request.headers:
        headers["sec-roles"] = request.headers.get("Sec-Roles")
    gnurl = app.extensions["conf"].get(
        app.extensions["conf"].get("localgn", "urls"), "secproxytargets"
    )
    query = {
        "size": 1,
        "_source": {"includes": ["id"]},
    ...
    md = requests.post(
        gnurl + "srv/api/search/records/_search", json=query, headers=headers
    )
...
def geonetwork_preauth(gnurl):
    ...
    if preauth.status_code == 204:
        if "XSRF-TOKEN" in preauth.cookies:
            ...
            headers["sec-proxy"] = "true"
            me = requests.get(
                gnurl + "srv/api/me", cookies=preauth.cookies, headers=headers
            )
        ...
        if "XSRF-TOKEN" in preauth.cookies:
            me = requests.get(
                ...
                headers={
                    ...
                    "X-XSRF-TOKEN": preauth.cookies["XSRF-TOKEN"],
                },
            )
            if me.status_code != 200:
                return me.text
                ...
                if "id" in me.json():
                    ...
                    md = requests.post(
                        gnurl + "srv/api/search/records/_search",
                        json=query,
                        cookies=preauth.cookies,
                        headers={
                            "Content-Type": "application/json",
                    ...
                    for h in rep["hits"]["hits"]:
                        try:
                            ...
                            app.logger.error(
                                f"no title in md {h['_id']} ? no key {str(e)}"
                            )
                        ...
                        retval.append(
                            {
                                "_id": h["_id"],
                                "gnid": h["_source"]["id"],
                                "gaialink": h["isPublishedToAll"]
                                and h["_source"]["isHarvested"] != "true",
                                "title": title,
                            }
                        )
...
def geoserver_workspaces():
    ...
    """
    gsurl = app.extensions["conf"].get(
        app.extensions["conf"].get("localgs", "urls"), "secproxytargets"
    )
    workspaces = requests.get(gsurl + "rest/workspaces.json", headers=request.headers)
